...
export async function anthropicValidityHelper(finding: Finding): Promise<void> {
    for (const anthropicOccurrence of Object.values(finding.secretValue)) {
        if (!anthropicOccurrence.api_key) {
            continue;
        }

        const validationResult = await validateAnthropicCredentials(
            anthropicOccurrence.api_key
        );
